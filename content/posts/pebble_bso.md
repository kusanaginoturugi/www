---
title: "Pebbleアプリを公開しました"
date: 2015-10-27T12:02:46
slug: "pebble_bso"
categories: ["コンピューター"]
tags: ["IoT","Pebble","コンピューター"]
---


<a href="http://apps.getpebble.com/en_US/application/561cb30d2116352d6600003d" target="_blank">BSO Baseball Indicator</a>

<a href="https://github.com/kusanaginoturugi/bso" target="_blank">GitHubリポジトリ</a>

野球の審判のためのインジケータを作成しました。

ボール・ストライク・アウトをカウントして表示するだけのアプリです。

ぶっちゃけ、HelloWorldに毛が生えた程度のシロモノですが、実際に使うとかなり便利で、普通のインジケータより使い勝手のよいものになりました。
